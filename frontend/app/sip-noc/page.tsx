...
function Content() {
  ...
  const saveIssueDate = async (id: string, value: string) => {
    const prev = rows.find((r) => r.id === id)?.issueDate ?? null;
    setRows((rs) => rs.map((r) => (r.id === id ? { ...r, issueDate: value || null } : r)));
    try {
      setSavingId(id);
      await apiClient.patch(`/noc/${id}/issue-date`, { issueDate: value || null });
      toast.success(value ? 'Issue date saved' : 'Issue date cleared');
    } catch (e: any) {
      setRows((rs) => rs.map((r) => (r.id === id ? { ...r, issueDate: prev } : r)));
      toast.error(e?.response?.data?.message || 'Failed to save issue date');
    } finally {
      setSavingId(null);
    }
  };
  ...
  useEffect(() => {
    (async () => {
      try {
        setLoading(true);
        const res = await apiClient.get('/noc', { params: sessionId ? { sessionId } : {} });
    ...
    <div className="mx-auto max-w-6xl space-y-4 p-4 sm:p-6">
      <div className="flex flex-wrap items-end justify-between gap-3">
        <div>
          ...
          <p className="mt-1 text-sm text-gray-500">Students who have uploaded their No-Objection Certificate. Set the NOC issue date for each entry.</p>
        </div>
            ...
            <table className="min-w-[960px] w-full text-sm">
              <thead className="bg-gray-50">
                <tr>
                  ...
                  <th className="px-4 py-2 text-left font-semibold text-gray-700">NOC issue date</th>
              ...
              <tbody className="divide-y divide-gray-100">
                {filtered.map((r) => (
                  <tr key={r.id} className="hover:bg-gray-50">
                    ...
                    <td className="px-4 py-2 text-xs text-gray-600">{fmt(r.uploadedAt)}</td>
                    <td className="px-4 py-2">
                      <input
                        type="date"
                        value={r.issueDate ? r.issueDate.slice(0, 10) : ''}
                        onChange={(e) => saveIssueDate(r.id, e.target.value)}
                        disabled={savingId === r.id}
                        aria-label={`NOC issue date for ${r.studentName || 'student'}`}
                        className="rounded border border-gray-300 px-2 py-1 text-xs text-gray-900 disabled:opacity-60"
                      />
                    </td>
                    <td className="px-4 py-2">
